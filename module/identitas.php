...
<div class="card card-primary card-tabs" style="width:100%">
	<div class="card-header p-0 pt-1">
		<ul class="nav nav-tabs" id="custom-tabs-two-tab" role="tablist">
			...
			<li class="nav-item">
				<a class="nav-link" id="custom-tabs-two-input-tab" data-toggle="pill" href="#custom-tabs-two-input" role="tab" aria-controls="custom-tabs-two-input" aria-selected="false">Input Kelompok</a>
			</li>
      <li class="nav-item">
	...
	<div class="card-body">
        ...
        <form class="" action="index.php?page=id_kelompok" method="post">
          <div class="form-group">
            <table>
              ...
              <td><button class="btn btn-success" type="submit" name="submit">Cari</button></td>
            </table>
          </div>
        </form>
          ...
          <?php
            ...
            if (isset($_GET['kat'])){ //Fungsi Get Untuk Dashboard
                $search = $_GET['kat'];
                //Menggunakan Inner Join Untuk Mendapatkan Data Jenis Kelompok
                $query1 = "SELECT * FROM tb_id_kelompok INNER JOIN tb_jenis_kelompok  USING (id_jenis_kelompok)
                WHERE id_jenis_kelompok LIKE '%$search%' ORDER BY kecamatan,id_jenis_kelompok LIMIT $posisi,$batas";
                $dt = mysqli_query($koneksi, $query1);
              }
              elseif (isset($_POST['kec'])){ //Fungsi Get Untuk Dashboard
                  $search = $_POST['kec'];
                  //Menggunakan Inner Join Untuk Mendapatkan Data Jenis Kelompok
                  $query1 = "SELECT * FROM tb_id_kelompok INNER JOIN tb_jenis_kelompok  USING (id_jenis_kelompok)
                  WHERE kecamatan LIKE '%$search%' ORDER BY kecamatan,id_jenis_kelompok";
                  $dt = mysqli_query($koneksi, $query1);
                }
              elseif(isset($_POST['search'])) { // Fungsi Search
		            $search = $_POST['search'];
                //Menggunakan Inner Join Untuk Mendapatkan Data Jenis Kelompok
                $query1 = "SELECT * FROM tb_id_kelompok INNER JOIN tb_jenis_kelompok  USING (id_jenis_kelompok)
                WHERE id_jenis_kelompok LIKE '%$search%' OR nama_kelompok LIKE '%$search%' OR
                kecamatan LIKE '%$search%' OR nama_pembina LIKE '%$search%' OR jabatan_pembina LIKE '%$search%'
                ORDER BY kecamatan,id_jenis_kelompok LIMIT $posisi,$batas";
                $dt = mysqli_query($koneksi, $query1);
              }
              else {
                //Menggunakan Inner Join Untuk Mendapatkan Data Jenis Kelompok
                $query1 = "SELECT * FROM tb_id_kelompok INNER JOIN tb_jenis_kelompok  USING (id_jenis_kelompok) order by kecamatan LIMIT $posisi,$batas";
                $dt = mysqli_query($koneksi, $query1);
              }
              if (mysqli_num_rows($dt)== 0 ) { //Mengecek Total Data Apabila 0 Maka Data Tidak Ditemukan
	                 echo "<center><h2>Maaf Data Tidak Ditemukan</center>";
                        }
              else {
